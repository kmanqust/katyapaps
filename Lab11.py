...
class TinkoffAPI(BankAPI):
    def getInfo(self, card_id):
        conn = sqlite3.connect('Tinkoff.db')
        cursor = conn.cursor()
        cursor.execute(f"SELECT * FROM Bank WHERE card_id = {card_id}")
        result = cursor.fetchall()
        conn.commit()
...
class APIFactory:
    def create(self, bankname: str):
        if bankname == 'Tinkoff':
            return TinkoffAPI()
        elif bankname == 'Sberbank':
            return SberBankAPI()
...
API1 = apifactory.create('Tinkoff')
API2 = apifactory.create('Sberbank')
